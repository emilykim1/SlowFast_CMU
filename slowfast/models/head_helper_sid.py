...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        for pathway in range(self.num_pathways):
            if pool_size[pathway] is None:
                avg_pool = nn.AdaptiveAvgPool3d((8, 1, 1)) # GAO
            else:
                avg_pool = nn.AdaptiveAvgPool3d((8, 1, 1)) # GAO
        ...
        #self.conv1 = nn.Conv3d(sum(dim_in), num_classes, kernel_size=(1, 1, 1))

        # GAO: 20220119 add FC layer
        self.projection = nn.Linear(18432, 64, bias=True)
    ...
    def forward(self, inputs):
        ...
        # # conv1
        # # (N, C, T, H, W) -> (N, T, H, W, C). for nn.linear
        ...
        # #x = self.projection(x)
        # x = self.conv1(x)
        # x = torch.squeeze(x, 3) # (N, C, T, H, W) -> (N, C, T)
        # x = torch.squeeze(x, 3) 
        ...
        # GAO: 20210119 add FC layer
        # (N, C, T, H, W) -> (N, T, H, W, C). for nn.linear
        x = x.permute((0, 2, 3, 4, 1))  # actually no need as long as T=C
        # Perform dropout.
        if hasattr(self, "dropout"):
            x = self.dropout(x)
        x = self.projection(x.reshape((x.shape[0], 18432)))
        x = x.view((x.shape[0], 8, 8))  # (N, T, C)

        # (N, T, C) -> (N, C, T)
        x = x.permute((0, 2, 1))  # actually no need as long as T=C     
...
class TransformerBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if dropout_rate > 0.0:
            self.dropout = nn.Dropout(dropout_rate)
        self.projection = nn.Linear(dim_in, 6, bias=True)
