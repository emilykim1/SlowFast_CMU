...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        for pathway in range(self.num_pathways):
            if pool_size[pathway] is None:
                ...
                avg_pool = nn.AdaptiveAvgPool3d((8,1,1))
            else:
                # avg_pool = nn.AvgPool3d((1,7,7), (1,7,7)) # Kim
        ...
        # Perform FC in a fully convolutional manner. The FC layer will be
        # initialized with a different std comparing to convolutional layers.
        # self.projection = nn.Linear(65536, 64, bias=True)
    ...
    def forward(self, inputs):
        ...
        pool_out = []
        for pathway in range(self.num_pathways):
            # print(inputs[pathway].shape)
            m = getattr(self, "pathway{}_avgpool".format(pathway))
            pool_out.append(m(inputs[pathway]))
        x = torch.cat(pool_out, 1)
        # (N, C, T, H, W) -> (N, T, H, W, C).
        # x = x.permute((0, 2, 3, 4, 1)) # Kim
        # Perform dropout.
        # print(x.shape)
        if hasattr(self, "dropout"):
            x = self.dropout(x)
        # x = self.projection(x.reshape((x.shape[0], 65536)))
        x = self.conv1(x)
        print(x.shape)
